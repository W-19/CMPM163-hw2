...
Shader "Custom/TerrainFromTexture"
{
	...
	{   
		...
		_DisplacementAmt ("Displacement", Float) = 0.0
		_VerticalStretch ("Vertical Stretch", Float) = 1.0
		_GrassPoint("Grass Point", Range(0, 1)) = 0.5
	}
	...
	SubShader
	{
		Pass {
			...
			uniform float _GrassPoint; // Point in vertical scale at which the texture is 100% grass
		   ...
		   { 
				...
				newPosition.y += (newPosition.y - v.vertex.y) * _VerticalStretch; // I added this to make it more interesting
		   ...
		   fixed4 frag(v2f i) : SV_Target
		   {
				...
				if (i.heightVal < _GrassPoint) {
				
					textureColor = lerp(dirt, grass, i.heightVal*(1/_GrassPoint));    
				}
				
				if (i.heightVal >= _GrassPoint) {
					textureColor = lerp(grass, snow, i.heightVal*(1/_GrassPoint) - 1);   
				}
